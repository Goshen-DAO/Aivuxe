...
export default function TokenPage({ nft, contractMetadata }: Props) {
  ...
  // Connect to NFT Collection smart contract using dynamicNFTCollectionAddress
  const { contract: nftCollection } = useContract(dynamicNFTCollectionAddress);
  ...
  // Fetch a list of valid direct listings
  const { data: directListing, isLoading: loadingDirect } =
    useValidDirectListings(marketplace, {
      tokenContract: dynamicNFTCollectionAddress,
  ...
  async function createBidOrOffer() {
    ...
    if (auctionListing?.[0]) {
      ...
      txResult = await marketplace?.offers.makeOffer({
        assetContractAddress: dynamicNFTCollectionAddress,
        tokenId: nft.metadata.id,
        totalPrice: bidValue,
      });
    } else {
  ...
  // Declaration of random colors within the component
...
export const getStaticProps: GetStaticProps = async (context) => {
  try {
    const { tokenId, contractAddress } = context.params as {
      tokenId: string;
      contractAddress: string;
    };

    const sdk = new ThirdwebSDK(NETWORK, {
      secretKey: process.env.SECRET_THIRDWEB_API_KEY,
    });

    // Use dynamicNFTCollectionAddress based on the contractAddress from the URL
    const dynamicNFTCollectionAddress = contractAddress;

    const contract = await sdk.getContract(dynamicNFTCollectionAddress);
    ...
    } else if (contract.erc1155) {
      // Use ERC1155 methods
      nft = await contract.erc1155.get(tokenId);
    } else {
      // Handle unsupported contract type
      throw new Error('Unsupported NFT contract type');
    }
    ...
    console.error('Error fetching NFT or contract metadata:', error);

    // Set default values for nft and contractMetadata in case of an error
    return {
      props: {
        nft: {},
...
export const getStaticPaths: GetStaticPaths = async (context: any) => {
  ...
  // Extract contract address from the URL path
  const dynamicNFTCollectionAddress = context.params?.contractAddress as string;

  if (!dynamicNFTCollectionAddress) {
    // Handle invalid or missing contractAddress
    return { paths: [], fallback: 'blocking' };
  }

  try {
    // Fetch the contract for the provided address
    const contract = await sdk.getContract(dynamicNFTCollectionAddress);
    let nfts;
    ...
    const paths = nfts.map((nft) => ({
      params: {
        contractAddress: dynamicNFTCollectionAddress,
    ...
    console.error('Error fetching contract or NFTs:', error);
    // Handle the error, maybe redirect or show an error page
    return { paths: [], fallback: 'blocking' };
  }
};
